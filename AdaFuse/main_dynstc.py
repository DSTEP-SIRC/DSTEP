...
def inner_main(argv):
    ...
    # TODO YD: define optimization policy for spatial gating
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.skip_training and not test_mode:
            ...
            train_usage_str = train(train_loader, model, criterion, optimizer, epoch, base_model_gflops, gflops_list,
                                    g_meta, my_criterion, p_anneal, args)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, base_model_gflops, gflops_list, g_meta, my_criterion,
          p_anneal, args):
    batch_time, data_time, closses, rlosses, blosses, losses, top1, top5 = get_average_meters(8)
    losses_dict = {}
    ...
    for i, input_tuple in enumerate(train_loader):
        ...
        if args.gpus is None:
            input_var_list = [torch.autograd.Variable(input_item).cpu() for input_item in
                              input_tuple[:-1]]
        ...
        if 'dynstc' in args.arch:
            output, mask_stack_list, _, _, spatial_masks_list, dyn_outputs = \
        ...
        if args.ada_reso_skip:
            # TODO YD Temp: Kept as reference
            ...
            # TODO YD: make the following loss the criterion for optimization
            closs, rloss, bloss = my_criterion(output, target[:, 0], dyn_outputs['flops_real'], dyn_outputs['flops_mask'],
            ...
            upb_batch_gflops_list.append(upb_flops_fixed.detach())
            real_batch_gflops_list.append(real_flops_fixed.detach())
        else:
        ...
        dynstc_loss = closs.mean() + rloss.mean() + bloss.mean()
        closses.update(closs.mean().item(), batchsize)
        rlosses.update(rloss.mean().item(), batchsize)
        blosses.update(bloss.mean().item(), batchsize)
        ...
        if i % args.print_freq == 0:
            # TODO YD: REPLACE ADAFUSE TRAIN PRINT
            ...
            print_output += ' {header:s} ({loss.avg:.3f})'.format(header='Bou', loss=blosses)
...
def validate(val_loader, model, criterion, epoch, base_model_gflops, gflops_list, g_meta, exp_full_path, my_criterion, args):
    batch_time, closses, rlosses, blosses, losses, top1, top5 = get_average_meters(7)
    all_results = []
    all_targets = []
    ...
    with torch.no_grad():
        for i, input_tuple in enumerate(val_loader):
            ...
            if args.ada_reso_skip:
                ...
                upb_batch_gflops_list.append(upb_flops_fixed)
                real_batch_gflops_list.append(real_flops_fixed)
            else:
            ...
            if i % args.print_freq == 0:
                # TODO YD: Remove, orig print from AdaFuse
...
# -j
# 0
# --exp_header sthv2_8_resnet18_dynstc_1024_gsmx_g.1_tsn_b16_lr.002
...
# 1
# -A
# "somethingv2 RGB --arch res18_dynstc_net --num_segments 8 --lr 0.002 --lr_steps 20 40 --epochs 50 --wd 5e-4 --npb --ada_reso_skip --init_tau 0.67 --gsmx --gate_history --gate_hidden_dim 1024 --gbn --grelu --gate_gflops_loss_weight 0.1 --gflops_loss_type upb --batch-size 16 -j 8 --gpus 0 --spatial_masking --den_target 0.6 --exp_header sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002"

# Resume [sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002] from epoch 9
# -d
# gitlab-srv:4567/ishay/cbi:ub18_cu100_py376_pytorch120
# -C
# execute
...
# 1
# -A
# "somethingv2 RGB --arch res18_dynstc_net --num_segments 8 --lr 0.002 --lr_steps 20 40 --epochs 50 --wd 5e-4 --npb --ada_reso_skip --init_tau 0.67 --gsmx --gate_history --gate_hidden_dim 1024 --gbn --grelu --gate_gflops_loss_weight 0.1 --gflops_loss_type upb --batch-size 16 -j 8 --gpus 0 --spatial_masking --den_target 0.6 --base_pretrained_from g0103-084653_sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002/models/ckpt.latest.pth.tar --start-epoch 9 --exp_header sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002"

# Resume 3rd time [sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002] from epoch 27
# -d
# gitlab-srv:4567/ishay/cbi:ub18_cu100_py376_pytorch120
# -C
# execute
...
# 1
# -A
# "somethingv2 RGB --arch res18_dynstc_net --num_segments 8 --lr 0.002 --lr_steps 20 40 --epochs 50 --wd 5e-4 --npb --ada_reso_skip --init_tau 0.67 --gsmx --gate_history --gate_hidden_dim 1024 --gbn --grelu --gate_gflops_loss_weight 0.1 --gflops_loss_type upb --batch-size 16 -j 8 --gpus 0 --spatial_masking --den_target 0.6 --base_pretrained_from g0104-114500_sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002/models/ckpt.latest.pth.tar --start-epoch 27 --exp_header sthv2_8_res18_dynstc_net_1024_gsmx_g.1_tsn_den.6_b16_lr.002"