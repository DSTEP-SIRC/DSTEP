...
class TSN_Gate(nn.Module):
    ...
    def forward(self, *argv, **kwargs):
        ...
        spatial_masks_list = None # YD: for "dynstc" arch
        ...
        if "cgnet" in self.args.arch:
            ...
            if self.args.policy_attention:
                ...
                feat, mask_stack_list = self.base_model(input_data.view(_b * _t, _c, _h, _w),
                                                        tau=kwargs["tau"], is_training=is_training, curr_step=curr_step)
        elif  "dynstc" in self.args.arch:
            # TODO YD: check if need policy attention + spatial masking support
            if self.args.policy_attention:
                feat, mask_stack_list, attention = self.base_model(input_data.view(_b * _t, _c, _h, _w),
                                                        tau=kwargs["tau"], is_training=is_training, curr_step=curr_step)
            else:
                feat, mask_stack_list, spatial_masks_list, dyn_outputs = self.base_model(input_data.view(_b * _t, _c, _h, _w),
                                                        tau=kwargs["tau"], is_training=is_training, curr_step=curr_step)
                # Reshape flops and norms data in "dyn_outputs" dict
                batch_size = _b
                frames_per_clip = _t
                ...
                dyn_outputs["flops_real"][2] = torch.Tensor([self.flops_fc]) # -> [scalar]: flops (per clip) of the FC layer (always static)
                dyn_outputs["norm_s"] = dyn_outputs["norm_s"].view(batch_size, frames_per_clip, -1) # -> [BatchSize, FramesPerClip, #ResNetLayers]
                # dyn_outputs["flops_real"][1] : [scalar]: flops (per 1 frame) of the FIRST CONVOLUTION in the network (always static)
                # dyn_outputs["norm_s_t"] :  [1, #ResNetLayers]: number of elements in the generated spatial mask at each ResNetLayer
                # dyn_outputs["flops_mask"] : [1, #ResNetLayers]: flops (per 1 frame) of the spatial mask generating conv at each ResNetLayer
        ...
        if "dynstc" in self.args.arch:
            if self.args.save_meta_gate:
                return output, mask_stack_list, None, torch.stack([base_out], dim=1), spatial_masks_list, dyn_outputs
            else:
                return output, mask_stack_list, None, torch.stack([base_out], dim=1), spatial_masks_list, dyn_outputs
        else:
            if self.args.save_meta_gate:
